...
export default function Home() {
  return <main>
    ...
    <section className="facts"><div><small>RPC URL</small><code>https://rpc.testnet.arc.io</code></div><div><small>CHAIN ID</small><strong>5042002</strong></div><div><small>도구</small><strong>Foundry</strong></div><div><small>결과</small><strong>Counter 컨트랙트</strong></div></section>
    ...
    <div className="content">
      ...
      <article>
        ...
        <section id="configure"><p className="step">STEP 2</p><h2>Arc 테스트넷 RPC 연결하기</h2><p>프로젝트 최상위 폴더에 <code className="inline">.env</code> 파일을 만들고 RPC 주소를 저장합니다.</p><Code>ARC_TESTNET_RPC_URL=&quot;https://rpc.testnet.arc.io&quot;</Code><p>저장한 값을 현재 터미널에 불러옵니다.</p><Code>source .env</Code><div className="callout danger"><b>보안 주의</b><p><code>.env</code>에는 이후 개인키가 들어갑니다. GitHub 등 공개 저장소에 절대 올리지 마세요.</p></div></section>
        ...
        <section id="deploy"><p className="step">STEP 4</p><h2>지갑을 만들고 테스트넷에 배포하기</h2><p>학습용 새 지갑을 만듭니다. 출력되는 개인키는 누구에게도 공유하면 안 됩니다.</p><Code>cast wallet new</Code><p><code className="inline">.env</code>에 개인키를 넣고 다시 불러옵니다.</p><Code>{`PRIVATE_KEY="0x..."\nsource .env`}</Code><div className="callout info"><b>가스비 받기</b><p><a href="https://faucet.circle.com" target="_blank" rel="noreferrer"><u>Circle Faucet</u></a>에서 Arc Testnet을 선택하고 지갑 주소를 입력해 테스트넷 USDC를 받으세요. Arc에서는 USDC가 가스 토큰입니다.</p></div><p>아래 명령으로 Counter 컨트랙트를 배포합니다.</p><Code>{`forge create src/Counter.sol:Counter \\\n  --rpc-url $ARC_TESTNET_RPC_URL \\\n  --private-key $PRIVATE_KEY \\\n  --broadcast`}</Code><p>출력의 <code className="inline">Deployed to</code> 주소를 저장하고 불러옵니다.</p><Code>{`COUNTER_ADDRESS="0x..."\nsource .env`}</Code><h3>Explorer에 소스 코드 검증하기</h3><p>Arc Testnet Explorer는 Blockscout를 사용합니다. 다음 명령으로 배포 주소에 소스 코드를 연결합니다.</p><Code>{`forge verify-contract $COUNTER_ADDRESS src/Counter.sol:Counter \\\n  --chain-id 5042002 \\\n  --verifier blockscout \\\n  --verifier-url https://testnet.arcscan.app/api/`}</Code></section>
        <section id="interact"><p className="step">STEP 5</p><h2>배포한 Counter 실행하기</h2><p>먼저 현재 숫자를 읽습니다. 새 Counter의 값은 <code className="inline">0</code>입니다.</p><Code>{`cast call $COUNTER_ADDRESS "number()(uint256)" \\\n  --rpc-url $ARC_TESTNET_RPC_URL`}</Code><p>온체인 트랜잭션을 보내 숫자를 1 증가시킵니다.</p><Code>{`cast send $COUNTER_ADDRESS "increment()" \\\n  --rpc-url $ARC_TESTNET_RPC_URL \\\n  --private-key $PRIVATE_KEY`}</Code><p>첫 번째 <code className="inline">cast call</code>을 다시 실행해 값이 <code className="inline">1</code>인지 확인하세요.</p><div className="complete"><span>✓</span><h3>첫 Arc 컨트랙트 배포 완료</h3><p><a href="https://testnet.arcscan.app" target="_blank" rel="noreferrer">Arc Testnet Explorer ↗</a>에서 주소나 트랜잭션 해시를 검색할 수 있습니다.</p></div></section>
